    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-blue-900 flex items-center justify-center p-4">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="w-full bg-white/50 rounded-xl p-4 mb-8 border border-white/5 text-left">
            <div className="flex justify-between items-center mb-3">
              <span className="text-blue-400 text-sm">Plano Atual</span>
              <span className="text-white font-medium">R$ {Number(valorPlano || 100).toFixed(2).replace('.', ',')}/mês</span>
            </div>
            <div className="flex justify-between items-center mb-3">
              <span className="text-blue-400 text-sm">Vencimento</span>
              <span className="text-blue-400 font-medium">{dataFormatada}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-blue-400 text-sm">Status</span>
              <span className="bg-blue-500/20 text-blue-400 text-xs px-2 py-1 rounded-md font-medium border border-blue-500/20">
                BLOQUEADO
              </span>
            </div>
          </div>

          <p className="text-blue-400 text-sm mb-4">Escolha como deseja renovar:</p>
          ...
          <div className="w-full border-t border-white/10 pt-6">
            <button 
              onClick={handleSair}
              className="flex items-center justify-center gap-2 w-full text-blue-400 hover:text-white transition-colors text-sm"
            >
      ...
      {modalPixAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-blue-800 border border-blue-700 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden transform transition-all">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setModalPixAberto(false)} className="text-blue-400 hover:text-white">
                  ✕
                </button>
              </div>

              <div className="space-y-4">
                <div className="bg-blue-900 p-4 rounded-xl border border-blue-700 text-center">
                  <span className="block text-blue-400 text-xs mb-1">Valor da Renovação</span>
                ...
                <div className="bg-blue-900 p-4 rounded-xl border border-blue-700">
                  <span className="block text-blue-400 text-xs mb-1">Recebedor</span>
                  <span className="block text-sm text-white font-medium">{nomePix}</span>
                </div>

                <div>
                  <span className="block text-blue-400 text-xs mb-2">Chave PIX (Celular/CPF/Email/Aleatória)</span>
                  <button 
                    onClick={copiarPix}
                    className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all ${copiado ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400' : 'bg-blue-900 border-blue-700 text-blue-300 hover:bg-blue-700'}`}
                  >
            ...
            <div className="p-4 border-t border-blue-700 bg-blue-800/50">
              <button 
                ...
                className="w-full bg-blue-700 hover:bg-blue-600 text-white font-medium py-3 rounded-xl transition-colors text-sm"
              >
                Enviar Comprovante
              </button>
            </div>
