    ...
    <div className="flex items-center gap-2">
      {[1,2,3,4].map(i => (
        <div key={i} className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
          i < step ? 'bg-emerald-400' : i === step ? 'bg-sky-400 scale-125 shadow-lg shadow-sky-400/40' : 'bg-gray-100'
        }`} />
      ))}
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-sky-50 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-6 animate-fadeIn">
          <div className="w-16 h-16 bg-gradient-to-br from-sky-400 to-sky-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-xl shadow-sky-500/20">
            <span className="text-2xl">✂️</span>
          </div>
          <h1 className="text-2xl font-black text-gray-800">Configuração do seu Salão</h1>
          <p className="text-gray-500 text-sm mt-1">Etapa {step} de 4 — {STEP_TITLES[step]}</p>
          <div className="flex justify-center mt-4"><Dots /></div>
        </div>

        {/* Card */}
        <div className="bg-white backdrop-blur-xl rounded-2xl border border-gray-200 shadow-2xl overflow-hidden animate-wizard-slide">
          <div className="p-6 space-y-5">
            ...
            {step === 2 && (<>
              ...
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Seus custos mensais:</p>
              {custos.map((c, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input className="flex-1 bg-gray-50 border border-gray-200 text-gray-800 rounded-lg px-3 py-2 text-sm outline-none focus:border-sky-400"
                  ...
                  <div className="relative w-28">
                    ...
                    <input type="number" className="w-full bg-gray-50 border border-gray-200 text-gray-800 rounded-lg pl-8 pr-2 py-2 text-sm outline-none focus:border-sky-400 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              ...
              <button onClick={addCusto} className="text-sm text-gray-500 font-bold hover:text-gray-600">+ Adicionar outro custo</button>
            ...
            {/* ═══ ETAPA 3 — Equipe ═══ */}
            {step === 3 && (<>
              <p className="text-sm text-gray-600">Você (Proprietária) já está na lista! 👑</p>
              <div className="bg-sky-500/10 border border-sky-500/20 rounded-xl p-3 flex items-center gap-3">
                <span className="text-lg">👑</span>
                <div>
                  <p className="text-gray-800 font-bold text-sm">{(nomeProprietaria || nomeSalao).toUpperCase()}</p>
                  <p className="text-gray-600 text-xs">Proprietária</p>
                </div>
              </div>

              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mt-2">Tem mais alguém na equipe?</p>
              {equipe.map((e, i) => (
                <div key={i} className="bg-gray-50 border border-gray-200 rounded-xl p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <select className="bg-gray-50 border border-gray-200 text-gray-800 rounded-lg px-2 py-2 text-sm outline-none"
              ...
              <button onClick={addProfissional} className="text-sm text-gray-500 font-bold hover:text-gray-600">+ Adicionar profissional</button>
            </>)}

            {/* ═══ ETAPA 4 — Serviços (opcional) ═══ */}
            {step === 4 && (<>
              <p className="text-xs text-gray-500">Clique para adicionar (pode adicionar mais depois em Precificação):</p>

              <p className="text-xs font-bold text-gray-600 uppercase tracking-wider">Cabelo:</p>
                  ...
                  return (
                    <button key={chip.nome} onClick={() => toggleServico(chip, 'CABELO')}
                      className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                        sel ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/20 animate-chip-pop'
              ...
              <p className="text-xs font-bold text-gray-600 uppercase tracking-wider mt-2">Estética:</p>
              <div className="flex flex-wrap gap-2">
              ...
              {servicosSelecionados.length > 0 && (
                <div className="space-y-3 mt-2">
                  {servicosSelecionados.map(s => (
                    <div key={s.nome} className="bg-gray-50 border border-gray-200 rounded-xl p-3">
                      <p className="text-gray-800 font-bold text-xs mb-2">{s.tipo === 'CABELO' ? '✂️' : '💅'} {s.nome} — Qual o preço{s.tipo === 'CABELO' ? ' do Curto (P)' : ''}?</p>
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <span className="absolute left-2 top-2 text-gray-500 text-sm">R$</span>
                          <input type="number" step="0.01" className="w-28 bg-gray-50 border border-gray-200 text-gray-800 rounded-lg pl-8 pr-2 py-2 text-sm outline-none focus:border-sky-400 font-bold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            value={s.precoP} onChange={e => updatePrecoServico(s.nome, e.target.value)} />
                        </div>
                        {s.tipo === 'CABELO' && (
                          <div className="text-xs text-gray-500">
                            <span>Médio: <b className="text-gray-600">{fmt(s.precoP * 1.20)}</b> (+20%)</span>
                            <span className="ml-2">Longo: <b className="text-gray-600">{fmt(s.precoP * 1.30)}</b> (+30%)</span>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {servicosSelecionados.length === 0 && (
                <div className="bg-sky-500/10 border border-sky-500/20 rounded-xl p-3 mt-2">
                  <p className="text-gray-600 text-xs">💡 Você pode adicionar seus serviços depois em <b>Precificação</b>.</p>
                </div>
              )}
            </>)}
              ...
              <button onClick={salvarEtapa} disabled={loading}
                className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg disabled:opacity-50 ${
                  step === 4
                    ...
                    : 'bg-gradient-to-r from-sky-500 to-sky-500 text-white shadow-sky-500/20 hover:from-sky-600 hover:to-sky-600'
                }`}>
                {loading ? (
  ...
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)}
        ...
        className="w-full bg-gray-50 border border-gray-200 text-gray-800 rounded-xl px-4 py-3 text-sm outline-none focus:border-sky-400 focus:bg-white transition-all placeholder:text-gray-400 font-medium [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
    </div>
  );
}
