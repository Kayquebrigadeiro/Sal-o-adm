...
export default function Configuracoes({ salaoId, role }) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto animate-fadeIn">
      ...
      {role === 'PROPRIETARIO' && (
        <div className="flex gap-4 mb-8 border-b border-slate-700 overflow-x-auto custom-scrollbar">
          <button
            onClick={() => setAbaAtiva('equipe')}
            className={`px-4 py-3 font-bold uppercase transition-all whitespace-nowrap border-b-2 text-sm ${abaAtiva === 'equipe' ? 'border-slate-900 text-white' : 'border-transparent text-slate-400 hover:text-slate-200 hover:bg-slate-950'}`}
          >
            Equipe & Parceiros
          </button>
          ...
          <button
            ...
            className={`px-4 py-3 font-bold uppercase transition-all whitespace-nowrap border-b-2 text-sm ${abaAtiva === 'plano' ? 'border-slate-900 text-white' : 'border-transparent text-slate-400 hover:text-slate-200 hover:bg-slate-950'}`}
          >
            Meu Plano
          </button>
          */}
      ...
      {abaAtiva === 'equipe' && (
        ...
        {profissionais.map(prof => (
          <div key={prof.id} className="bg-slate-900 p-5 rounded-3xl border border-slate-800 shadow-sm flex items-center justify-between group hover:border-slate-300 transition-all">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg ${prof.cargo === 'PROPRIETARIO' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-200'}`}>
      ...
      <Modal open={modalProf} onClose={() => setModalProf(false)} title={editando ? "EDITAR PROFISSIONAL" : "NOVO PROFISSIONAL"}>
        <div className="space-y-4">
          <div>
            <label className="text-sm font-bold text-slate-100 mb-1 block uppercase">Nome Completo</label>
            <input
              type="text" className="w-full border-2 border-slate-800 p-3 rounded-2xl focus:border-slate-900 outline-none transition-all"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-bold text-slate-100 mb-1 block uppercase">Cargo / Papel</label>
              <select
                className="w-full border-2 border-slate-800 p-3 rounded-2xl focus:border-slate-900 outline-none uppercase"
            ...
            <div>
              <label className="text-sm font-bold text-slate-100 mb-1 block uppercase">Salário Fixo (Opcional)</label>
              <div className="relative">
                <span className="absolute left-3 top-3.5 text-slate-400 text-sm">R$</span>
                <input
                  type="number" className="w-full border-2 border-slate-800 p-3 pl-10 rounded-2xl focus:border-slate-900 outline-none"
      ...
      {abaAtiva === 'plano' && role === 'PROPRIETARIO' && (
        <div className="bg-slate-900 rounded-3xl border border-slate-700 p-8 shadow-sm max-w-2xl mx-auto animate-fadeIn">
          ...
          {loadingAssinatura ? (
            <div className="flex items-center justify-center py-12">
               <span className="animate-spin w-8 h-8 border-4 border-slate-700 border-t-slate-900 rounded-full"></span>
            </div>
          ) : erroAssinatura ? (
            ...
            <div className="space-y-6">
               <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800">
                    ...
                    <p className="font-black text-white text-lg">{fmt(100)} <span className="text-xs font-bold text-slate-400 uppercase">/ mês</span></p>
                  </div>
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800">
                  ...
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800">
                    ...
                    <p className={`font-black text-lg ${assinatura.dias_restantes < 0 ? 'text-rose-600' : 'text-white'}`}>
      ...
      <Modal open={modalPix} onClose={() => setModalPix(false)} title="RENOVAR MEU PLANO">
        <div className="space-y-6">
           ...
           <div>
             <p className="text-xs font-black text-slate-200 uppercase tracking-widest mb-3">Pague via PIX:</p>
             
             {pixCopiaCola && (
               <div className="flex flex-col items-center justify-center p-4 bg-slate-900 rounded-2xl border-2 border-slate-700 mb-4 shadow-sm animate-fadeIn">
                 <img 
                   src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(pixCopiaCola)}`}
                   alt="QR Code PIX"
                   className="w-40 h-40 sm:w-48 sm:h-48 rounded-xl shadow-sm border border-slate-800"
                 />
                 <p className="text-[10px] font-black text-slate-400 uppercase mt-4 tracking-widest">Escaneie com o app do banco</p>
               </div>
             )}

             <div className="bg-slate-900 border-2 border-slate-700 rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-center justify-between">
               ...
               <button 
                 onClick={copiarChave}
                 className="w-full sm:w-auto bg-slate-100 text-slate-100 px-5 py-3 rounded-xl text-xs font-black uppercase hover:bg-slate-200 transition-colors flex items-center justify-center gap-2 whitespace-nowrap flex-shrink-0"
               >
