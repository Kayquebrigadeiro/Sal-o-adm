  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-blue-900 flex items-center justify-center p-4">
      <div className="bg-white/[0.05] border border-white/10 backdrop-blur-xl p-8 rounded-2xl shadow-2xl max-w-md w-full relative overflow-hidden">
        ...
        <div className="absolute -top-32 -left-32 w-64 h-64 bg-sky-500/20 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute -bottom-32 -right-32 w-64 h-64 bg-sky-500/20 rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 flex flex-col items-center text-center">
          
          <div className="w-16 h-16 bg-sky-500/20 rounded-2xl flex items-center justify-center mb-6 border border-sky-500/30">
            <AlertTriangle className="w-8 h-8 text-gray-500" />
          </div>

          <h1 className="text-2xl font-bold text-gray-800 mb-2 tracking-tight">Assinatura Inativa</h1>
          ...
          <div className="w-full bg-white/50 rounded-xl p-4 mb-8 border border-white/5 text-left">
            <div className="flex justify-between items-center mb-3">
              <span className="text-gray-500 text-sm">Plano Atual</span>
              <span className="text-gray-800 font-medium">R$ {Number(valorPlano || 100).toFixed(2).replace('.', ',')}/mês</span>
            </div>
            <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="bg-sky-500/20 text-gray-500 text-xs px-2 py-1 rounded-md font-medium border border-sky-500/20">
                BLOQUEADO
              </span>
            </div>
          </div>

          <p className="text-gray-500 text-sm mb-4">Escolha como deseja renovar:</p>
          ...
          <div className="grid grid-cols-2 gap-3 w-full mb-8">
            ...
            <button 
              ...
              className="flex flex-col items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-gray-800 p-4 rounded-xl transition-all border border-white/5"
            >
          ...
          <div className="w-full border-t border-white/10 pt-6">
            <button 
              onClick={handleSair}
              className="flex items-center justify-center gap-2 w-full text-gray-500 hover:text-gray-800 transition-colors text-sm"
            >
      ...
      {modalPixAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-sky-500 border border-gray-200 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden transform transition-all">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setModalPixAberto(false)} className="text-gray-500 hover:text-gray-800">
                  ✕
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-white p-4 rounded-xl border border-gray-200 text-center">
                  <span className="block text-gray-500 text-xs mb-1">Valor da Renovação</span>
                  <span className="block text-2xl font-bold text-gray-800">R$ {Number(valorPlano || 100).toFixed(2).replace('.', ',')}</span>
                </div>

                <div className="bg-white p-4 rounded-xl border border-gray-200">
                  <span className="block text-gray-500 text-xs mb-1">Recebedor</span>
                  <span className="block text-sm text-gray-800 font-medium">{nomePix}</span>
                </div>

                <div>
                  <span className="block text-gray-500 text-xs mb-2">Chave PIX (Celular/CPF/Email/Aleatória)</span>
                  <button 
                    onClick={copiarPix}
                    className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all ${copiado ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-400' : 'bg-white border-gray-200 text-gray-600 hover:bg-sky-500'}`}
                  >
              ...
              <div className="mt-6 bg-sky-500/10 border border-sky-500/20 rounded-xl p-4 flex gap-3">
            ...
            <div className="p-4 border-t border-gray-200 bg-sky-500/50">
              <button 
                onClick={() => {
                  setModalPixAberto(false);
                  abrirWhatsApp();
                }}
                className="w-full bg-sky-500 hover:bg-sky-500 text-white font-medium py-3 rounded-xl transition-colors text-sm"
              >
                Enviar Comprovante
              </button>
            </div>
